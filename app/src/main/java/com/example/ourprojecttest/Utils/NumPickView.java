...
public class NumPickView extends View {
        ...
        for (int i = 0; i < ta.getIndexCount(); i++) {
            ...
            switch (index) {
                case R.styleable.NumPickView_totalNum:
                    ...
                    if(total>10) {
                        for (int j = 10; j < total; j++) {
                                mData.add(String.valueOf(j));
                        }
                    }
                    else {
                        for(int j=0;j<=9;j++)
                            mData.add("."+String.valueOf(j)+" 厘米");
                    }
                    break;
                case R.styleable.NumPickView_totalNum2:
                    int total1 = ta.getInteger(index, 24);

                    if(total1>10) {
                        for (int j = 10; j < total1; j++) {
                            mData.add(String.valueOf(j));
                        }
                    }
                    else {
                        for(int j=0;j<=9;j++)
                            mData.add("."+String.valueOf(j)+" 千克");
                    }
                    break;
                case R.styleable.NumPickView_source:
                    mData.add("主任医师");
                    mData.add("副主任医师");
                    mData.add("主治医师");
                    mData.add("医师");
                    mData.add("暂无");
                    break;
                case R.styleable.NumPickView_offices:
                    mData.add("全科");
                    mData.add("眼科");
                    mData.add("口腔科");
                    mData.add("耳鼻喉科");
    ...
    public String getData(){
        String s=(String)mData.get(mCurrentPostion);
        return  s;
    }
}
